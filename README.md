...
The e2e tests deploy kubexit in a real Kubernetes cluster to verify behavior against the live Kubernetes API.

#### Local testing with Kind

The easiest way to run e2e tests locally is using [Kind](https://kind.sigs.k8s.io/) (Kubernetes in Docker). Requires Docker and the `kind` binary installed.
...
# Run the tests
cd ci/e2e-test/client-server
./apply-job.sh      # Deploy the test job
